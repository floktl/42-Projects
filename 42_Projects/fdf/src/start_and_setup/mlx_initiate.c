...
/*   Updated: 2024/03/24 14:29:45 by fkeitel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	initialize_mlx_image(t_window *window)
{
	...
	if (!(window->image)
		...
		return (EXIT_FAILURE);
	window->image2 = mlx_new_image(window->mlx, WIDTH, HEIGHT);
	if (!(window->image)
		|| mlx_image_to_window(window->mlx, window->image, 0, 0) == ERROR)
		return (EXIT_FAILURE);
	...
	window->image2 = mlx_put_string(window->mlx, "Press <I> for manual", 8, 8);
	return (EXIT_SUCCESS);
}
